...
const CollectionView = {
  ...
  async render(container) {
    ...
    container.innerHTML = `
      <div class="collection-view" id="collection-scroll">
        ...
        <div class="collection-grid" id="collection-grid">
          ${this._renderGrid(this._getActiveLetters(myLetters, discovered, capsules))}
        </div>
    ...
    this._bindEvents(container, { myLetters, discovered, capsules });
    ...
    if (letters.length === 0) {
      ...
      return `
        <div class="collection-empty">
          <div class="collection-empty-icon">${this._activeTab === 'sent' ? '📮' : this._activeTab === 'capsules' ? '⏳' : '🗺️'}</div>
  ...
  _bindEvents(container, { myLetters, discovered, capsules }) {
    ...
    container.querySelector('#btn-collection-sort').addEventListener('click', () => {
      ...
      this._refreshGrid(myLetters, discovered, capsules);
    });
  ...
  _refreshGrid(myLetters, discovered, capsules) {
    const grid = document.getElementById('collection-grid');
    if (!grid) return;
    const letters = this._getActiveLetters(myLetters, discovered, capsules);
